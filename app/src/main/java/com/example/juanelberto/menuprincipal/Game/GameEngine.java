...
import com.example.juanelberto.menuprincipal.Game.characters.Bonk;
...
public class GameEngine {
    ...
    private void physics(int delta) {
        // Player physics
        //Cuando pulsa la p el juego se queda siempre pausado.
        if(input.isPause()){
            return;
        }
    ...
    void draw(Canvas canvas) {
        ...
        canvas.drawText("||", 90, 8, paint);

        canvas.drawText("score: "+bonk.getScore()+"", 0, 8, paintScore);
        canvas.drawText("Live: x"+bonk.getLive()+"", 70, 8, paintScore);
        if(bonk.getLive() < 1){
            canvas.drawText("Game Over", 30,50, paintScore);
        }

        canvas.drawText("Speed: x"+bonk.getVx()+"",0,20,paintScore);
